...
export async function GET(req: Request) {
  ...
  if (bold && cache700) {
    return fontResponse(cache700.file, cache700.contentType)
  }
  if (!bold && cache400) {
    return fontResponse(cache400.file, cache400.contentType)
  }

  const { file, contentType } = readFontFile(`heebo-hebrew-${weight}-normal`)
